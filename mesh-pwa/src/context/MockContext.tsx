...
type MockContextType = {
    ...
    toggleSaveCampaign: (id: string) => Promise<void>;
    deleteCampaign: (id: string) => Promise<void>;
    addCampaignUpdate: (campaignId: string, content: string, imageUrl?: string, title?: string) => Promise<void>;
    shareCampaignToChat: (campaign: Campaign, targetThreadId?: string, targetUserId?: string) => Promise<string>;
    hiddenCampaignIds: string[];
    viewedCampaignIds: string[];
    hideCampaign: (id: string) => void;
    unhideAllCampaigns: () => void;
    markCampaignViewed: (id: string) => void;
    markAllCampaignsViewed: () => void;
    hasUnreadActiveCampaigns: boolean;
    rides: Ride[];
    postRide: (data: {
        origin: string;
        destination: string;
...
export function MockDataProvider({ children }: { children: ReactNode }) {
    ...
    const [hiddenCampaignIds, setHiddenCampaignIds] = useState<string[]>(() => {
        if (typeof window !== "undefined") {
            try {
                const stored = localStorage.getItem("yogheart_hidden_campaigns");
    ...
    const [viewedCampaignIds, setViewedCampaignIds] = useState<string[]>(() => {
        if (typeof window !== "undefined") {
            try {
                const stored = localStorage.getItem("yogheart_viewed_campaigns");
                return stored ? JSON.parse(stored) : [];
            } catch (e) {
                return [];
            }
    ...
    // ===== CROWDFUNDING / HEARTHFUND FEATURE =====

    // Subscribe to campaigns collection (all active fundraisers)
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(campaignsRef, (snapshot) => {
            if (!snapshot.empty) {
                const list: Campaign[] = snapshot.docs.map(d => {
                    const data = d.data();
                    const targetGoal = Number(data.targetGoal) || 1000;
                    ...
                    return {
                        ...
                        organizerColor: data.organizerColor || "bg-emerald-200",
                        ...
                        needsJumpstart: !!data.needsJumpstart || (amountRaised < targetGoal * 0.2),
                        createdAt: data.createdAt || new Date().toISOString(),
                        savedBy: Array.isArray(data.savedBy) ? data.savedBy : [],
            ...
            console.error("MockContext: onSnapshot campaigns error:", error);
            setCampaigns(INITIAL_CAMPAIGNS);
        });
    ...
    const createCampaign = async (data: {
        ...
        if (!currentUserId) throw new Error("Must be logged in to create a campaign");
        ...
        if (coverUrl.startsWith("data:")) {
            try {
                const storagePath = `campaigns/${currentUserId}/${campaignId}_cover`;
                coverUrl = await uploadImageToStorage(coverUrl, storagePath);
            } catch (err) {
                console.warn("Cover upload failed, using dataUrl fallback:", err);
            }
        }
        ...
        if (data.images && data.images.length > 0) {
            for (let i = 0; i < data.images.length; i++) {
                ...
                if (img.startsWith("data:")) {
                    try {
                        const storagePath = `campaigns/${currentUserId}/${campaignId}_img_${i}`;
                        const url = await uploadImageToStorage(img, storagePath);
                        uploadedImages.push(url);
                    } catch (err) {
                        uploadedImages.push(img);
        ...
        // Extract state code if formatted e.g. "Boston, MA" -> "MA"
        const parts = data.location.split(",");
        const stateCode = parts.length > 1 ? parts[parts.length - 1].trim().slice(0, 2).toUpperCase() : "";

        const newCampaign: Campaign = {
            id: campaignId,
            organizerId: currentUserId,
            organizerName: currentUser.name || "Me",
            organizerAvatar: currentUser.avatar || null,
            organizerColor: currentUser.color || "bg-emerald-200",
            organizerRole: data.organizerRole?.trim() || `Organized by ${currentUser.name || "Organizer"}`,
            title: data.title.trim(),
            story: data.story.trim(),
            category: data.category || "Community",
            location: data.location.trim() || "USA",
            stateCode: stateCode,
            targetGoal: Number(data.targetGoal) || 1000,
            amountRaised: 0,
            supportersCount: 0,
            coverImage: coverUrl || "https://images.unsplash.com/photo-1532629345422-7515f3d16bb9?w=800&auto=format&fit=crop&q=80",
        ...
        try {
            ...
            addNotification("Fundraiser campaign published! 🤝🎉");
        } catch (err) {
            console.error("Failed to save campaign to Firestore:", err);
            addNotification("Fundraiser created locally.");
        }
    ...
    const donateToCampaign = async (
        ...
        const newDonation: CampaignDonation = {
            id: donationId,
            donorId: isAnonymous ? undefined : (currentUserId || undefined),
            donorName: isAnonymous ? "Anonymous Supporter" : (currentUser?.name || "Kind Supporter"),
            donorAvatar: isAnonymous ? null : (currentUser?.avatar || null),
        ...
        const updatedAmount = target.amountRaised + amount;
        const updatedSupporters = target.supportersCount + 1;
        const updatedDonations = [newDonation, ...(target.donations || [])];
        const needsJumpstart = updatedAmount < target.targetGoal * 0.2;

        setCampaigns(prev => prev.map(c => c.id === campaignId ? {
            ...c,
            amountRaised: updatedAmount,
            supportersCount: updatedSupporters,
            donations: updatedDonations,
            needsJumpstart: needsJumpstart
        } : c));
        ...
        try {
            ...
            addNotification(`Thank you! Your donation of $${amount.toFixed(2)} was received! ❤️🤝`);
        } catch (err) {
            console.error("Failed to update campaign donation in Firestore:", err);
            addNotification(`Donation of $${amount.toFixed(2)} recorded! ❤️`);
        }
    };
    ...
    const toggleSaveCampaign = async (id: string) => {
        ...
        setCampaigns(prev => prev.map(c => c.id === id ? { ...c, savedBy: newSavedBy } : c));
        ...
        try {
            ...
            addNotification(isSaved ? "Campaign removed from saved" : "Campaign saved to your favorites! ❤️");
        } catch (err) {
    ...
    const deleteCampaign = async (id: string) => {
        ...
        try {
            ...
            addNotification("Campaign removed.");
        } catch (err) {
            console.error("Failed to delete campaign:", err);
        }
    };

    const addCampaignUpdate = async (campaignId: string, content: string, imageUrl?: string, title?: string) => {
        const target = campaigns.find(c => c.id === campaignId);
        if (!target) return;

        let finalImgUrl = imageUrl;
        if (imageUrl && imageUrl.startsWith("data:") && currentUserId) {
            try {
                const updateId = crypto.randomUUID();
                finalImgUrl = await uploadImageToStorage(imageUrl, `campaigns/${currentUserId}/update_${updateId}`);
            } catch (err) {
                console.warn("Update image upload failed:", err);
            }
        }
        ...
        const updatedUpdates = [newUpdate, ...(target.updates || [])];
        ...
        try {
            ...
            addNotification("Campaign update posted! 📢");
        } catch (err) {
            console.error("Failed to post campaign update:", err);
        }
    };

    const shareCampaignToChat = async (campaign: Campaign, targetThreadId?: string, targetUserId?: string): Promise<string> => {
        if (!currentUserId) throw new Error("Must be logged in to share campaign");
        ...
        const progressPercent = Math.min(Math.round((campaign.amountRaised / campaign.targetGoal) * 100), 100);
        const text = `🤝 Campaign: ${campaign.title}\n📍 ${campaign.location} • $${campaign.amountRaised.toLocaleString()} raised of $${campaign.targetGoal.toLocaleString()} (${progressPercent}%)\n\n"${campaign.story.slice(0, 140)}..."`;
        ...
        await sendMessage(threadId, text, "text", {
            ...
            replyTo: {
                id: campaign.id,
                sender: "them",
                senderId: campaign.organizerId,
                text: `HeartFund: ${campaign.title} ($${campaign.amountRaised.toLocaleString()} raised)`
            }
        });

        addNotification("Campaign shared to chat! 💬🤝");
        return threadId;
    };

    const hideCampaign = (id: string) => {
        setHiddenCampaignIds(prev => {
            const next = Array.from(new Set([...prev, id]));
            if (typeof window !== "undefined") {
                try {
                    localStorage.setItem("yogheart_hidden_campaigns", JSON.stringify(next));
                } catch (e) {}
            }
            return next;
        });
    ...
    const unhideAllCampaigns = () => {
        setHiddenCampaignIds([]);
        if (typeof window !== "undefined") {
            try {
                localStorage.removeItem("yogheart_hidden_campaigns");
            } catch (e) {}
        }
    };

    const markCampaignViewed = (id: string) => {
        setViewedCampaignIds(prev => {
            if (prev.includes(id)) return prev;
            const next = [...prev, id];
            if (typeof window !== "undefined") {
                try {
                    localStorage.setItem("yogheart_viewed_campaigns", JSON.stringify(next));
                } catch (e) {}
            }
            return next;
        });
    ...
    const markAllCampaignsViewed = () => {
        const allActiveIds = campaigns
            .filter(c => c.status === "active" && !hiddenCampaignIds.includes(c.id))
            .map(c => c.id);
        setViewedCampaignIds(prev => {
            const next = Array.from(new Set([...prev, ...allActiveIds]));
            if (typeof window !== "undefined") {
                try {
                    localStorage.setItem("yogheart_viewed_campaigns", JSON.stringify(next));
                } catch (e) {}
    ...
    const hasUnreadActiveCampaigns = useMemo(() => {
        return campaigns.some(c => (c.status === "active" || !c.status) && !hiddenCampaignIds.includes(c.id) && !viewedCampaignIds.includes(c.id));
    }, [campaigns, hiddenCampaignIds, viewedCampaignIds]);
    ...
    const contextValue = useMemo(() => ({
            ...
            unhideAllCampaigns,
            markCampaignViewed,
            markAllCampaignsViewed,
            hasUnreadActiveCampaigns,
            rides,
            unlockedRideIds,
            unlockRide,
            postRide,
        ...
        transactions, moods, callLogs, marketplaceItems, campaigns, rides, unlockedRideIds, drafts,
